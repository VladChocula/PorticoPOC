    ...
    try {
        do {
            ...
            if (!features || features.length === 0) {
                if (offset === 0) {
                    console.log(`\nNo addresses returned for ${muniId}`);
                    console.log(`Response: ${JSON.stringify(response.data, null, 2)}`);
                }
                break;
            }

            const batch = features.map((feature: any) => {
        ...
        offset += batchSize;
        console.log(`\nFetched ${fetched} rows at offset ${offset}, total so far: ${allResults.length}`);
...
export async function batchUploadToSupabase(tableName: string, data: NJGISAddress[]): Promise<void> {
    console.log(`Starting upload: ${data.length} rows -> ${tableName}`);
    ...
    for (let i = 0; i < data.length; i += BATCH_SIZE) {
        ...
        const { error } = await supabase
            ...
            .insert(chunk);
        if (error) {
            console.error(`❌ Error inserting batch ${i / BATCH_SIZE + 1} (${chunk.length} rows) → ${tableName}:`, error);
        } else {
            console.log(`✅ Inserted batch ${i / BATCH_SIZE + 1} (${chunk.length} rows) → ${tableName}`);
        }
    }

    console.log(`Finished upload: ${tableName}`);
}
...
async function main() {
    ...
    for (const code of municipTable) {
        console.log(`\nFetching addresses for ${code.muni_fips}...`);
        const addrData = await getNJAddressesByMunicipality(code.muni_fips);

        console.log(`\n=== NJ County ${code.muni_fips} Addresses ===`);
        console.log(`\nTotal rows: ${addrData.length}`);

        await batchUploadToSupabase(`nj_${code.muni_fips.slice(0,2)}_prop_addr`, addrData);
    }
}
